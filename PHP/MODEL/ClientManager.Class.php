...
class ClientManager
{
    public static function add(Client $obj)
    {
        var_dump($obj);
        $db = DbConnect::getDb();
        $q = $db->prepare("INSERT INTO clients (idClient,nomClient,prenomClient,adresseClient,idVille) VALUES (:idClient,:nomClient,:prenomClient,:adresseClient,:idVille)");
    ...
    {
        $db = DbConnect::getDb();
        $q = $db->prepare("UPDATE clients SET  nomClient=:nomClient, prenomClient=:prenomClient, adresseClient=:adresseClient, idVille=:idVille WHERE idclient=:idClient");
        $q->bindValue(":idClient", $obj->getIdClient());
        $q->bindValue(":nomClient", $obj->getNomClient());
        $q->bindValue(":prenomClient", $obj->getPrenomClient());
        $q->bindValue(":adresseClient", $obj->getAdresseClient());
